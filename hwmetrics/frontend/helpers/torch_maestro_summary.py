...
def summary(model, input_size, batch_size=-1, device="cuda", silent=True):
    ...
    def register_hook(module):
        ...
        def hook(module, input, output):
            ...
            pa = re.compile(".*Attention")
            key =  module.__module__
            match_pc = pc.match(key)
            match_pl = pl.match(key)
            match_pa = pa.match(class_name)

            # attention
            if match_pa:
                B, N, C = input[0].shape
                nhead = module.sample_num_heads
                d = module.sample_qk_embed_dim
                inc = int(d / nhead)
                for i in range(nhead):
                    ...
                    K, C = N, inc
                    X, Xo, Y, Yo, R, S = N,N,1,1,1,1
                    summary[m_key]["stride"] = None
                    summary[m_key]["dimension_ic"] = (batch_size, K, C, R, S, Y, X)
                    summary[m_key]["dimension_oc"] = (batch_size, K, C, R, S, Yo, Xo)
            ...
            if not match_pc and isinstance(module, nn.Conv2d):
                match_pc = True
            if not match_pl and isinstance(module, nn.Linear):
                match_pl = True
            
            if (match_pc or match_pl) and not isinstance(module, nn.Identity):
